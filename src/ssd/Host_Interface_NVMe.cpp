...
// 将IO请求拆分成若干个事务，并存入user_request的Transaction_list中
void Input_Stream_Manager_NVMe::segment_user_request(User_Request *user_request)
{
	...
	while (handled_sectors_count < req_size)
	{
		...
		// access_status_bitmap变量在很多地方都用到了，但我就是没看懂这个啥意思（^v^)，还有sector相关的问题
...
// 结果xml文件中<SSDDevice.HostInterface>部分的数据来源
