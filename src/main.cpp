...
void command_line_args(char* argv[], string& input_file_path, string& workload_file_path)
{
	...
	for (int arg_cntr = 1; arg_cntr < 5; arg_cntr++) {
		...
		if (arg.compare(0, strlen(file_path_switch), file_path_switch) == 0) {			  // -i则为SSD配置文件路径
		...
		if (arg.compare(0, strlen(workload_path_switch), workload_path_switch) == 0) {	  // -w则为工作负载定义文件路径
...
void read_configuration_parameters(const string ssd_config_file_path, Execution_Parameter_Set* exec_params)
{
	...
	if (!ssd_config_file) {									// 如果SSD配置文件未指定，则将生成默认的SSD配置文件
		...
		exec_params->XML_serialize(xmlwriter);				// 写入默认的SSD配置参数
		xmlwriter.Close();
		PRINT_MESSAGE("[====================] Done!\n")
	} else {
		...
		if (line.compare("USE_INTERNAL_PARAMS") != 0) {
			...
			rapidxml::xml_node<> *mqsim_config = doc.first_node("Execution_Parameter_Set");		// 将<Execution_Parameter_Set>当作配置文件的开头
			if (mqsim_config != NULL) {
				exec_params = new Execution_Parameter_Set;
				exec_params->XML_deserialize(mqsim_config);										// 读取SSD配置文件参数信息									
			} else {
...
std::vector<std::vector<IO_Flow_Parameter_Set*>*>* read_workload_definitions(const string workload_defs_file_path)
{
	...
	bool use_default_workloads = true;								// 是否使用默认的负载定义参数
	if (!workload_defs_file) {
		...
		if (line.compare("USE_INTERNAL_PARAMS") != 0) {
			...
			strcpy(temp_string, line.c_str());
			doc.parse<0>(temp_string);
			rapidxml::xml_node<> *mqsim_io_scenarios = doc.first_node("MQSim_IO_Scenarios");		// 将<MQSim_IO_Scenarios>当作配置文件的开头
			if (mqsim_io_scenarios != NULL) {
				for (auto xml_io_scenario = mqsim_io_scenarios->first_node("IO_Scenario"); xml_io_scenario; xml_io_scenario = xml_io_scenario->next_sibling("IO_Scenario")) {
					...
					for (auto flow_def = xml_io_scenario->first_node(); flow_def; flow_def = flow_def->next_sibling()) {
						// 对每一个<IO_Flow_Parameter_Set_Synthetic>或<IO_Flow_Parameter_Set_Trace_Based>中的内容进行以下操作
						IO_Flow_Parameter_Set* flow;												
						// 给IO_Flow_Parameter_Set类赋值，有可能是IO_Flow_Parameter_Set_Synthetic子类，也有可能是 IO_Flow_Parameter_Set_Trace_Based子类
...
void collect_results(SSD_Device& ssd, Host_System& host, const char* output_file_path)
{
	...
	xmlwriter.Write_open_tag(tmp);
	// 分别调用Report_results_in_XML函数，输出模拟过程中的统计信息
...
int main(int argc, char* argv[])
{
	...
	if (argc != 5) {
		// MQSim命令示例： MQSim.exe -i ssdconfig.xml -w workload.xml
	...
	command_line_args(argv, ssd_config_file_path, workload_defs_file_path);			// 解析命令行参数，给ssd_config_file_path, workload_defs_file_path赋值

	Execution_Parameter_Set* exec_params = new Execution_Parameter_Set;
	read_configuration_parameters(ssd_config_file_path, exec_params);				// 给Execution_Parameter_Set类赋值
	/*
	给IO_Flow_Parameter_Set类赋值，使用二维数组接收参数是因为一个负载定义文件可能有多个<IO_Scenario>，
	一个<IO_Scenario>可能有多个<IO_Flow_Parameter_Set_Synthetic>或<IO_Flow_Parameter_Set_Trace_Based>
	*/
	std::vector<std::vector<IO_Flow_Parameter_Set*>*>* io_scenarios = read_workload_definitions(workload_defs_file_path);

	int cntr = 1;
	// 有多少个<IO_Scenario>,就要模拟多少次
